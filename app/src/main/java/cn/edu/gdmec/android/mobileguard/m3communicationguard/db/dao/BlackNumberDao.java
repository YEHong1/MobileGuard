...
public class BlackNumberDao {
    private BlackNumberOpenHelper blackMumberOpenHelper;
    public BlackNumberDao(Context context){
        super();
        blackMumberOpenHelper=new BlackNumberOpenHelper(context,"blacknumber.db",null,1);
...
public boolean add(BlackContactInfo blackContactInfo){
    ...
    if (blackContactInfo.phoneNumber.startsWith("+86")){
        ...
        values.put("mytype",blackContactInfo.mytype);
...
public List<BlackContactInfo> getPageBlackNumber(int pagenumber, int pagesize){
    ...
    Cursor cursor=db.rawQuery("select number,mode,name,mytype from blacknumber limit ? offset ?",
   ...
   List<BlackContactInfo> mBlackContactInfos=new ArrayList<BlackContactInfo>();
    while(cursor.moveToNext()){
        ...
        info.mytype=cursor.getString(3);
